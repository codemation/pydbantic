...
async def test_model_filtering_operators(loaded_database_and_model):
    ...
    big_salary_employee = await Employees.filter(Employees.gte("salary", 50000))
    assert len(big_salary_employee) == 1

    mid_salary_employee = await Employees.filter(
        Employees.gte("salary", 30000), Employees.lte("salary", 40000)
    )
    assert len(mid_salary_employee) == 2

    low_and_high_salary = await Employees.filter(
        Employees.lt("salary", 20000), Employees.gt("salary", 40000)
    )
    ...
    employees_with_salary = await Employees.filter(
        Employees.gt("salary", 0), order_by=Employees.asc("salary")
    )
    ...
    employees_with_salary = await Employees.filter(
        Employees.gt("salary", 0), order_by=Employees.desc("salary")
    )
    for i, employee in enumerate(employees_with_salary):
        if i == 0:
            continue
    ...
    mid_salary_employees = await Employees.filter(
        Employees.salary >= 30000, Employees.salary <= 40000
    )
    ...
    init_employees = await Employees.filter(Employees.salary >= 30000)
    second_employees = await Employees.filter(Employees.salary.inside([20000, 40000]))

    mid_salary_employees = await Employees.filter(
        Employees.OR(
            [Employees.salary >= 30000], [Employees.salary.inside([20000, 40000])]
        ),
