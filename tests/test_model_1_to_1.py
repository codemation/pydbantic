...
from tests.models import Department, Employee, EmployeeInfo
...
async def test_model_1_to_1(loaded_database_and_model):
    ...
    positoin_in_department = await employee_info.employee.position[
        0
    ].department.positions[0]()
    assert (
        positoin_in_department.department.department_id
        == employee_info.employee.position[0].department.department_id
    )
